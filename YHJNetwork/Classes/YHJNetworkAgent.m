...
- (AFHTTPRequestSerializer *)requestSerializerForRequest:(YHJBaseRequest *)request {
    AFHTTPRequestSerializer *requestSerializer = nil;
    if (request.requestSerializerType == YHJRequestSerializerTypeHTTP) {
        requestSerializer = [AFHTTPRequestSerializer serializer];
    } else if (request.requestSerializerType == YHJRequestSerializerTypeJSON) {
...
- (NSURLSessionTask *)sessionTaskForRequest:(YHJBaseRequest *)request error:(NSError **)error {
    YHJRequestMethod method = [request requestMethod];
    ...
    switch (method) {
        ...
        case YHJRequestMethodHEAD:
            return [self dataTaskWithHTTPMethod:@"HEAD" requestSerializer:requestSerializer URLString:url parameters:param error:error];
        case YHJRequestMethodPUT:
            return [self dataTaskWithHTTPMethod:@"PUT" requestSerializer:requestSerializer URLString:url parameters:param error:error];
        case YHJRequestMethodDELETE:
            return [self dataTaskWithHTTPMethod:@"DELETE" requestSerializer:requestSerializer URLString:url parameters:param error:error];
        case YHJRequestMethodPATCH:
...
- (void)addRequest:(YHJBaseRequest *)request {
    ...
    if ([request.requestTask respondsToSelector:@selector(priority)]) {
        switch (request.requestPriority) {
            case YHJRequestPriorityHigh:
                request.requestTask.priority = 1.0;
                break;
            case YHJRequestPriorityLow:
                request.requestTask.priority = 0;
                break;
            case YHJRequestPriorityDefault:
                /*!!fall through*/
            default:
...
- (void)cancelAllRequests {
    ...
    if (allKeys && allKeys.count > 0) {
        ...
        for (NSNumber *key in copiedKeys) {
            Lock();
            YHJBaseRequest *request = _requestsRecord[key];
            Unlock();
...
- (BOOL)validateResult:(YHJBaseRequest *)request error:(NSError * _Nullable __autoreleasing *)error {
    BOOL result = [request statusCodeValidator];
    if (!result) {
        if (error) {
            *error = [NSError errorWithDomain:YHJRequestValidationErrorDomain code:YHJRequestValidationErrorInvalidStatusCode userInfo:@{NSLocalizedDescriptionKey:@"Invalid status code"}];
        }
        return result;
    }
...
- (void)handleRequestResult:(NSURLSessionTask *)task responseObject:(id)responseObject error:(NSError *)error {
    Lock();
    YHJBaseRequest *request = _requestsRecord[@(task.taskIdentifier)];
    Unlock();
...
- (void)requestDidSucceedWithRequest:(YHJBaseRequest *)request {
    @autoreleasepool {
...
- (void)addRequestToRecord:(YHJBaseRequest *)request {
    Lock();
